...
define(["mootools", "BVTouchable","Tangle"], function($,BVTouchable,Tangle) {
...
//---------------------------------------------------------- // Added by Julian
//
//  TKAdjustableNumberWithDataSource
//
//  Drag a number to adjust.
//
//  Attributes:  data-min (optional): minimum value
...
// Depends on isAnyAdjustableNumberDragging hack for dragging one value over another one

// XXX: Careful! This currently assumes that the datasource is an array of unique values
Tangle.classes.TKAdjustableNumberWithDataSource = {

    initialize: function (element, options, tangle, variable, datasource) {
        this.element = element;
        this.tangle = tangle;
        this.variable = variable;
    ...
    touchDidGoDown: function (touches) {
        this.indexAtMouseDown = this.tangle.getValue(this.datasource).indexOf(this.tangle.getValue(this.variable));
        this.isDragging = true;
        isAnyAdjustableNumberDragging = true;
        this.updateRolloverEffects();
        this.updateStyle();
    ...
    touchDidMove: function (touches) {
        var index = this.indexAtMouseDown + touches.translation.x / 5 * this.step;
        index = ((index / this.step).round() * this.step).limit(this.min, this.max);
        this.tangle.setValue(this.variable, this.tangle.getValue(this.datasource)[index]);
        this.updateHelp();
    },
